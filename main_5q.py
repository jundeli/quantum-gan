...
#         z = torch.from_numpy(z).to(self.device).float()
        ...
        d_loss_real = -torch.mean(logits_real) # removed minus sign by junde
        ...
        if (i+1) % self.n_critic == 0:
            ...
            (edges_hat, nodes_hat) = self.postprocess((edges_logits, nodes_logits), self.post_method)
            logits_fake, features_fake = self.D(edges_hat, None, nodes_hat)
            g_loss_fake = -torch.mean(logits_fake) # removed minus sign by junde
...
#             try:
#                 R_sec_last, F_sec_last = R_last, F_last
#                 R_last, F_last = R, F
#             except:
#                 continue
...
#             try:
#                 fd_score = frdist(R+R_last+R_sec_last, F+F_last+F_sec_last)
#             except:
#                 fd_score = frdist(R+R+R, F+F+F)
#                 R_last, F_last = R, F

            # Logging.
            loss['G/loss_fake'] = g_loss_fake.item()
            loss['G/loss_value'] = g_loss_value.item()
            loss['fd_score'] = fd_score
            print('g_loss:{}, d_loss:{}, fd_score:{}'.format(g_loss.item(), d_loss.item(), fd_score))
        ...
        if (i+1) % self.log_step == 0:
            ...
            with open('molgan/results/q5_metric_scores_log.csv', 'a') as file:
        ...
        if (i+1) % self.model_save_step == 0:
            ...
            with open('molgan/models/q5_weights.csv', 'a') as file:
                writer = csv.writer(file)
