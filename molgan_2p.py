...
#         z = torch.stack(tuple(sample_list)).to(self.device).float()
        ...
        if (i+1) % self.n_critic == 0:
            ...
            if fd_bond_atom_min > fd_bond_atom:
                ...
                with open('molgan_2p/models/molgan_2p_red_weights.csv', 'a') as file:
                    writer = csv.writer(file)
                    writer.writerow([i+1] + list(gen_weights.detach().numpy()))
                with open('molgan_2p/models/lowest_indices.csv', 'a') as file:
                    writer = csv.writer(file)
        ...
        if (i+1) % self.log_step == 0:
            ...
            with open('molgan_2p/results/molgan_2p_red_metric_scores_log.csv', 'a') as file:
                writer = csv.writer(file)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_dir', type=str, default='molgan_2p/logs')
    parser.add_argument('--model_save_dir', type=str, default='molgan_2p/models')
    parser.add_argument('--sample_dir', type=str, default='molgan_2p/samples')
    parser.add_argument('--result_dir', type=str, default='molgan_2p/results')
