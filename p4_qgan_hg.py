...
def main(config):
    ...
    g_lr = self.g_lr
    d_lr = self.d_lr
    gen_weights = torch.tensor(list(np.random.rand(12)*2-1), requires_grad=True)
...
#         z = torch.from_numpy(z).to(self.device).float()
        ...
        if (i+1) % self.n_critic == 0:
            ...
            if fd_bond_atom_min > fd_bond_atom:
                ...
                with open('p4_qgan_hg_15p/models/p4_qgan_hg_15p_red_weights.csv', 'a') as file:
                    writer = csv.writer(file)
                    writer.writerow([i+1] + list(gen_weights.detach().numpy()))
                with open('p4_qgan_hg_15p/models/lowest_indices.csv', 'a') as file:
                    writer = csv.writer(file)
        ...
        if (i+1) % self.log_step == 0:
            ...
            with open('p4_qgan_hg_15p/results/q8_metric_scores_log.csv', 'a') as file:
                writer = csv.writer(file)
        ...
        if (i+1) % self.model_save_step == 0:
            ...
            torch.save(self.D.state_dict(), D_path)
            torch.save(self.V.state_dict(), V_path)
            with open('p4_qgan_hg_15p/models/q8_weights.csv', 'a') as file:
                writer = csv.writer(file)
                writer.writerow([i+1] + list(gen_weights.detach().numpy()))
